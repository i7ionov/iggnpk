...
export class ContributionsInfromationFormComponent implements OnInit {
  ...
  act() {
    ...
    let now = new Date();
    let mistakes = '';
    this.contrib_info.mistakes.forEach(function (item, i, arr) {
      if (i > 0){
        mistakes = mistakes + ', ';
      }
    ...
    mistakes = mistakes.replace(new RegExp('{house}', 'g'), `${this.contrib_info.notify.house.address.city}, ${this.contrib_info.notify.house.address.street}, д. ${this.contrib_info.notify.house.number}`);
